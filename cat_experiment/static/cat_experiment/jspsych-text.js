    ...
    jsPsych.text = (function() {
        ...
        plugin.create = function(params) {
            ...
            for (var i = 0; i < trials.length; i++) {
				...
				trials[i].timeout = params.timeout || -1 //defines time after which the trial automatically advances. default is infinite
            }
            return trials;
        };
        ...
        plugin.trial = function(display_element, block, trial, part) {
			...
			//If there is a timeout, start the countdown now
			var autoAdvance;
			if(trial.timeout>0){
				autoAdvance = setTimeout(function(){
					if(trial.cont_key=='mouse'){
						...
						after_response({key: "none", rt: trial.timeout});
					}
				}, trial.timeout);
			}

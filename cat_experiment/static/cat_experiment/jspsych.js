...
(function ($) {
    jsPsych = (function () {
        ...
        var exp_start_time;
        //NEW FEATURE: how many consecutive timeouts we have
        var consecutiveTimeouts = 0;
        //NEW FEATURE: how many total timeouts we have
        var totalTimeouts = 0;
        var waslastTrialMissed = false;
        ...
        function isFailed() {
            if (consecutiveTimeouts > opts.maxConsecutiveTimeouts || totalTimeouts > opts.maxTotalTimeouts) {
                return true;
            }
            else {
                return false;
        ...
        core.abort = function () {
            $(DOM_target).html("<p>Sorry! you were idle for too long or missed too many trials</p><br><p>Désolé! vous avez été inactif trop longtemps ou avez manqué trop d'essais</p>");
            $(DOM_target).append('<button class="reload" type="button">Try Again - Recommencer</button>');
            $("button.reload").click(function (e) {
                location.reload();
            });
            throw "experiment aborted";
        }

        function countTimeout() {
            totalTimeouts += 1;
            if (waslastTrialMissed) {
        ...
        function run() {
            ...
            for (var i = 0; i < exp_blocks.length; i++) {
                ...
                //gets an array of trial objects
                var trials = jsPsych[plugin_name]["create"].call(null, opts["experiment_structure"][i]);

                //takes the trial list and creates a block object
        ...
        function nextBlock() {
            //Globalish variable controlling the block index
            curr_block += 1;
        ...
        function createBlock(trial_list) {
            //Block object to be created
            var block = {
                trial_idx: -1,

                trials: trial_list,
                //Each block carries its own data. It is an array of key-value pairs (objects)
                data: [],
                ...
                writeData: function (data_object) {
                    //I guess this is the right place to increment private global data
                    if (data_object.hasOwnProperty('correct') && data_object.isPractice == false) {
                        if (data_object.correct == true) {
                            correctCounter++;
                        }
                        if (data_object.isPractice == false) {
                            opts.informProgress(correctCounter);
                        }
                ...
                timedOut: function () {
                    ...
                    waslastTrialMissed = true;
                    if (isFailed()) {
                        jsPsych.abort();
                    }
                },

                //Bravo Daniel, tes vrm nice :)
                allIsFine: function () {
        ...
        function do_trial(block, trial) {
            // execute trial method noted as plugin.trial in the plugin object with part=1
    ...
    jsPsych.pluginAPI = (function () {
        ...
        module.getKeyboardResponse = function (callback_function, valid_responses, rt_method, persist) {
            ...
            var listener_function = function (e) {
                ...
                //here would be the time to return if any excluded keys are specified
